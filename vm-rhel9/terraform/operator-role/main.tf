...
data "aws_iam_policy_document" "assume_role_trust" {
  # Trust only this deployment's EC2 role. Permit optional session tags on chaining.
  statement {
...
resource "aws_iam_role" "secrets_sync" {
  ...
  description        = "Role assumed by Vault Secrets Sync using its initial EC2 instance profile session."
...
data "aws_iam_policy_document" "secrets_manager" {
  statement {
    ...
    resources = [
      "arn:aws:secretsmanager:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:secret:vault-vm/*",
    ]
  }
}
...
resource "aws_iam_role_policy" "vault_irsa_assume_role" {
  name = "mapfre-vm-operator-role-assume"
