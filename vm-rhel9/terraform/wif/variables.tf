...
variable "public_oidc_issuer_url" {
  ...
  type        = string
  default     = "https://vault-vm.jose-merchan.sbx.hashidemos.io"
  nullable    = false
...
variable "vault_pod_role_name" {
  description = "EC2 instance profile role used by Vault; optional session tags are allowed for role chaining."
  type        = string
