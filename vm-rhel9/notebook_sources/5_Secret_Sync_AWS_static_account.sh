...
MOUNT=operator-role-kv
DEST=aws-sm-operator-role
KIND=aws-sm
...
cat > "$VM_ROOT/terraform/operator-role/runtime.auto.tfvars.json" <<EOF
{"vault_irsa_role_name":"$(jq -r .vault_role_name "$STATE/infrastructure.json")","sync_role_name":"mapfre-vm-operator-role","destination_name":"aws-sm-operator-role","aws_region":"$AWS_REGION"}
EOF
...
vault secrets list -format=json > "$STATE/operator-role-mounts.json"
if [[ -s "$VM_ROOT/terraform/operator-role/terraform.tfstate" ]] && ! jq -e --arg key "$MOUNT/" 'has($key)' "$STATE/operator-role-mounts.json" >/dev/null; then
  cp "$VM_ROOT/terraform/operator-role/terraform.tfstate" "$STATE/operator-role-before-recovery.tfstate"
  terraform -chdir="$VM_ROOT/terraform/operator-role" state list | grep -E '^vault_secrets_sync_association\.|^vault_generic_endpoint\.aws_destination$' > "$STATE/operator-role-stale-resources" || true
  while read -r resource; do
    [[ -z "$resource" ]] || terraform -chdir="$VM_ROOT/terraform/operator-role" state rm "$resource"
  done < "$STATE/operator-role-stale-resources"
fi
terraform -chdir="$VM_ROOT/terraform/operator-role" init -input=false
terraform -chdir="$VM_ROOT/terraform/operator-role" validate
terraform -chdir="$VM_ROOT/terraform/operator-role" apply -input=false -auto-approve
terraform -chdir="$VM_ROOT/terraform/operator-role" output -json > "$STATE/operator-role-outputs.json"
...
vault write "sys/sync/destinations/aws-sm/$DEST" region="$AWS_REGION" role_arn="$(terraform -chdir="$VM_ROOT/terraform/operator-role" output -raw sync_role_arn)" \
