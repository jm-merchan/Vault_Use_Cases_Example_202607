...
cat > "$VM_ROOT/terraform/irsa/runtime.auto.tfvars.json" <<EOF
{"vault_irsa_role_name":"$(jq -r .vault_role_name "$STATE/infrastructure.json")"}
EOF
...
vault secrets list -format=json > "$STATE/irsa-mounts.json"
if [[ -s "$VM_ROOT/terraform/irsa/terraform.tfstate" ]] && ! jq -e --arg key "$MOUNT/" 'has($key)' "$STATE/irsa-mounts.json" >/dev/null; then
  cp "$VM_ROOT/terraform/irsa/terraform.tfstate" "$STATE/irsa-before-recovery.tfstate"
  terraform -chdir="$VM_ROOT/terraform/irsa" state list | grep -E '^vault_secrets_sync_association\.|^vault_generic_endpoint\.aws_destination$' > "$STATE/irsa-stale-resources" || true
  while read -r resource; do
    [[ -z "$resource" ]] || terraform -chdir="$VM_ROOT/terraform/irsa" state rm "$resource"
  done < "$STATE/irsa-stale-resources"
fi
terraform -chdir="$VM_ROOT/terraform/irsa" init -input=false
terraform -chdir="$VM_ROOT/terraform/irsa" validate
terraform -chdir="$VM_ROOT/terraform/irsa" apply -input=false -auto-approve
terraform -chdir="$VM_ROOT/terraform/irsa" output -json > "$STATE/irsa-outputs.json"
...
vault write "sys/sync/destinations/aws-sm/$DEST" region="$AWS_REGION"  \
