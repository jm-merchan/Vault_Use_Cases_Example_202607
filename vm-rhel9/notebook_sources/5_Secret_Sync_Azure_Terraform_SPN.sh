...
cat > "$VM_ROOT/terraform/azure-spn/runtime.auto.tfvars.json" <<EOF
{"azure_subscription_id":"$AZURE_SUBSCRIPTION_ID","name_prefix":"mvm-spn"}
EOF
...
vault secrets list -format=json > "$STATE/azure-spn-mounts.json"
if [[ -s "$VM_ROOT/terraform/azure-spn/terraform.tfstate" ]] && ! jq -e --arg key "$MOUNT/" 'has($key)' "$STATE/azure-spn-mounts.json" >/dev/null; then
  cp "$VM_ROOT/terraform/azure-spn/terraform.tfstate" "$STATE/azure-spn-before-recovery.tfstate"
  terraform -chdir="$VM_ROOT/terraform/azure-spn" state list | grep -E '^vault_secrets_sync_association\.|^vault_generic_endpoint\.aws_destination$' > "$STATE/azure-spn-stale-resources" || true
  while read -r resource; do
    [[ -z "$resource" ]] || terraform -chdir="$VM_ROOT/terraform/azure-spn" state rm "$resource"
  done < "$STATE/azure-spn-stale-resources"
fi
terraform -chdir="$VM_ROOT/terraform/azure-spn" init -input=false
terraform -chdir="$VM_ROOT/terraform/azure-spn" validate
terraform -chdir="$VM_ROOT/terraform/azure-spn" apply -input=false -auto-approve
terraform -chdir="$VM_ROOT/terraform/azure-spn" output -json > "$STATE/azure-spn-outputs.json"
...
DEST=mvm-spn-azure-kv
KIND=azure-kv
cat "$VM_ROOT/terraform/azure-spn/main.tf"
KEY_VAULT=$(az keyvault list -g "mvm-spn-secrets-sync-rg" --query '[0].name' -o tsv)
